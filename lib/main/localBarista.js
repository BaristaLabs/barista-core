...
class Barista {
    ...
    brew(baristaContext, code, codePath, callback) {
        ...
        //Pull settings from the request.
        //If the request has a header named "X-Barista-Execution-Timeout" set the max execution timeout.
        var executionTimeout = _.get(baristaContext.request.headers, "x-barista-execution-timeout");
        executionTimeout = _.parseInt(executionTimeout);
        if (!_.isNaN(executionTimeout))
            baristaContext.executionTimeout = executionTimeout;
