...
codepress.brew = function (baristaContext, code, codePath, callback) {
    ...
    try {
        ...
        var maxExecutionTimeout = 5 * 60 * 1000; //5 minutes

        if (!_.isNaN(_.parseInt(process.env.npm_config_barista_maxExecutionTimeout)))
            maxExecutionTimeout = _.parseInt(process.env.npm_config_barista_maxExecutionTimeout);

        var executionTimeout = maxExecutionTimeout;
        if (_.isNumber(baristaContext.executionTimeout))
            executionTimeout = baristaContext.executionTimeout;
        
        if (executionTimeout > maxExecutionTimeout)
            throw "Execution Timeout cannot exceed the Maximum Defined Execution Timeout. Execution Timeout:" + executionTimeout + "ms Max Execution Timeout:" + maxExecutionTimeout + "ms.";
